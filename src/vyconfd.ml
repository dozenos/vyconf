...
let get_value world token (req: request_get_value) =
    try
        let () = (Lwt_log.debug @@ Printf.sprintf "[%s]\n" (Dozenos1x.Util.string_of_list req.path)) |> Lwt.ignore_result in
...
let get_values world token (req: request_get_values) =
    try
        ...
        let values_str =
         (match fmt with
          | Out_plain -> Dozenos1x.Util.string_of_list @@ List.map (Printf.sprintf "\'%s\'") values
          | Out_json -> Dozenos1x.Util.json_of_list values)
...
let list_children world token (req: request_list_children) =
    try
        ...
        let children_str =
          (match fmt with
          | Out_plain -> Dozenos1x.Util.string_of_list @@ List.map (Printf.sprintf "\'%s\'") children
          | Out_json -> Dozenos1x.Util.json_of_list children)
...
let make_world config dirs =
    let open Session in
    (* the reference_tree json file is generated at dozenos-1x build time *)
