module CT = Dozenos1x.Config_tree
module IC = Dozenos1x.Internal.Make(CT)
module CC = Commitd_client.Commit
module DT = Dozenos1x.Diff_tree
module DS = Dozenos1x.Diff_show
module UN = Dozenos1x.Union
module VT = Dozenos1x.Vytree
module VL = Dozenos1x.Vylist
module RT = Dozenos1x.Reference_tree
...
let string_of_op op =
    match op with
    | CfgSet (path, value, _) ->
        let path_str = Dozenos1x.Util.string_of_list path in
        (match value with
...
let set w s path =
    if Dozenos1x.Util.is_empty path
...
let copy w s p1 p2 =
    if Dozenos1x.Util.is_empty p1
    then raise (Session_error "Cannot copy an empty path")
    else
    if Dozenos1x.Util.is_empty p2
    ...
    then
    let p1_str = Dozenos1x.Util.string_of_list p1_total in
...
let rename w s p1 p2 =
    ...
    let p2_last =
      match Dozenos1x.Util.get_last p2 with
      | None -> raise (Session_error "Cannot rename to an empty value")
      | Some v -> v
    in
    if (Dozenos1x.Util.drop_last p1) <> (Dozenos1x.Util.drop_last p2)
...
let edit_env_str s =
    ...
    | _ as p ->
        Printf.sprintf
        "[edit %s]\\n\\u@\\H${VRF:+(vrf:$VRF)}${NETNS:+(ns:$NETNS)}# "
        (Dozenos1x.Util.string_of_list p)
    in
...
let set_edit_level_up w s =
    let current_level = s.edit_level in
    let new_level =
        if Dozenos1x.Util.is_empty current_level then
            current_level
        else
        let up = Dozenos1x.Util.drop_last current_level in
        match RT.allowed_edit_level w.reference_tree up with
        | Ok () -> up
        | Error _ -> (* tag_value *)
            Dozenos1x.Util.drop_last up
    in
    let session = { s with edit_level = new_level; }
    in session, (edit_env_str session)

let get_edit_level _w s =
    Dozenos1x.Util.string_of_list s.edit_level
...
let edit_level_root _w s =
    Dozenos1x.Util.is_empty s.edit_level
...
let load w s file cached =
    ...
    let ct =
        if cached then
            ...
            with Dozenos1x.Internal.Read_error e ->
                Error e
        else
            Dozenos1x.Config_file.load_config file
    in
    match ct with
...
let merge w s file destructive =
     ...
     *)
    let ct = Dozenos1x.Config_file.load_config file in
    match ct with
    | Error e -> raise (Session_error (Printf.sprintf "Error loading config: %s" e))
    | Ok config ->
...
let save w s file =
    ...
    let res = Dozenos1x.Config_file.save_config ct file in
    match res with
...
let config_unsaved w s file id =
    ...
    let res =
        try
            ...
            not (Dozenos1x.Util.file_compare ~ignore_line_prefix:"//" tmp_save file)
...
let get_completion_env ?(legacy_format=false) w s path =
    ...
    let path_total = s.edit_level @ path in
    let res =
        Dozenos1x.Completion.get_completion_env_str ~legacy_format w.reference_tree config op path_total
    in
    match res with
    | Ok env -> env
...
let write_running_cache w =
    ...
    try
        ...
        in Ok ()
    with
        Dozenos1x.Internal.Write_error msg ->
            let msg =
...
let get_config w s id =
    ...
    let () =
        ...
        with
            Dozenos1x.Internal.Write_error msg -> raise (Session_error msg)
    in
    let () =
        try
...
let show_config w s path =
    ...
    let path_show =
        ...
        | `Leaf_value -> Dozenos1x.Util.drop_last path_total
        | _ -> path_total
    in
    let proposed_config = get_proposed_config w s in
    if not (Dozenos1x.Util.is_empty path_show) &&
