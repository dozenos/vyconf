...
let lex_order c1 c2 =
    let c = Dozenos1x.Util.lex_order c1.path c2.path in
    match c with
    | 0 ->
        begin
            match c1.tag_value, c2.tag_value with
            | Some t1, Some t2 -> Dozenos1x.Util.lexical_numeric_compare t1 t2
            | _ -> 0
        end
    | _ as a -> a
...
let owner_args_from_data p o =
    ...
    let arg_value = Dozenos1x.Util.get_last_n p var_pos
    in owner, arg_value
...
let get_node_data rt ct src (path, cs') t =
     ...
     *)
    if Dozenos1x.Util.is_empty path then
        (path, cs')
    else
